...
predictions_url <- "https://raw.githubusercontent.com/openearth/sealevel/feature/predictions-mean-tide/data/deltares/results/dutch-sea-level-monitor-predictions-2023-2-28.csv"
# predictions_url <- "../../data/deltares/results/dutch-sea-level-monitor-predictions-2023-2-14.csv"
...
## SHOULD CORRECTION BE WITH MEASURED OR CORRECTED HEIGHT?
...
knmi_df21 <- read_csv("../../data/knmi/knmi21/knmi_21_5_50_95.csv") %>%
  filter(scenario == "ssp126", percentiles %in% c(5, 50, 95), proc == "Total") %>%
  mutate(correction = "knmi21") %>%
  mutate(perc_ts_nap = perc_ts + height_1986_2005)

knmi_df22 <- read_csv("../../data/knmi/knmi22/knmi_22_5_50_95.csv") %>%
  filter(scenario == "ssp126", percentiles %in% c(5, 50, 95), proc == "Total") %>%
  mutate(correction = "knmi22") %>%
  mutate(perc_ts_nap = perc_ts + height_1995_2014)
...
knmi_df <- knmi_df21 %>% bind_rows(knmi_df22)
  ...
  geom_ribbon(data = knmi_df %>% filter(percentiles != 50) %>%
    ...
    coord_cartesian(xlim = c(2000, 2040), ylim = c(0, 30)) +
     scale_color_manual(values = plotColors) +
    scale_fill_manual(values = plotFills) +
    xlab("jaar") +
    ylab("zeespiegel in cm tov NAP") +
