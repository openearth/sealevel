...
ggplot() +
    ...
    geom_line(data = . %>% filter(`windopzet correctie` == "NCEP1"), aes(year, prediction_recalc + anomaly, color = `windopzet correctie`), size = 1, alpha = 0.6) +
...
# generate plot for summary
  ...
  predictions_all %>% ungroup() %>%
    ...
    arrange(year) %>% 
    mutate(`correctie beschikbaar` = ifelse(year >= 1979, "jaarlijkse windopzetcorrectie", "gemiddelde windopzetcorrectie")) %T>%
    write_csv("dataGrafiekSamenvatting.csv") %>%
    ggplot() +
  ...
  annotate(geom = "text", x = 2003, y = 10, label = "2.9 mm/jaar", color = pal[2]) +
    ...
    labs(subtitle = "Zeespiegel, gecorrigeerd voor windopzet en nodaal getij \n gebroken lineair model") +
    fitstyle +
    theme(
      strip.text.y = element_text(angle = 0)) +
    theme(legend.title = element_blank()) +
...
ggsave(filename = "figures/samenvatting_v2.png", width = 5, height = 3.5, units = "in")
```
...
*   Marges - De bandbreedte van zeespiegelprojecties neemt toe naar de toekomst. Deze marges omvatten een combinatie van natuurlijke fluctuaties die op klimatologische tijdschalen plaatsvinden (bijvoorbeeld door langzame fluctuaties in oceaanstromingen of lokale warmteopslag) en door modelonzekerheid in de mate van lokale zeespiegelstijging voor een gegeven emissiescenario. Met name deze modelonzekerheid neemt toe met de tijd.
*   Versnelling - De zeespiegelversnelling in sommige projecties is toe te schrijven is aan processen die in een warmere toekomst een grotere rol kunnen spelen dan nu (zoals grootschalig massaverlies van de grote ijskappen).
...
Zowel het toevoegen van de autocorrelatieterm als de correctie voor heteroskedasticiteit (ongelijke verdeling van varianties) heeft geen effect op de schattingen van de huidige zeespiegel, alleen op de standaardfouten.
...
```{r, eval=FALSE}
  ...
  correctionColors = c(
    "yearly surge correction" = "black",
    "average surge correction" = "darkgrey"
  )
  ...
  predictions_all %>% ungroup() %>%
    ...
    filter(year >= startyear) %>% 
    arrange(year) %>% 
    mutate(`correctie beschikbaar` = ifelse(year >= 1979, "yearly surge correction", "average surge correction")) %>%
    ggplot() +
    geom_vline(xintercept = 1993, linetype = 4, alpha = 0.5, color = pal[3]) +
    geom_point(aes(year, (`height - anomaly` - `nodal tide`)/10, color = `correctie beschikbaar`), alpha = 0.6, size = symboolgrootte) +
    # geom_line(data = . %>% filter(model_variant == "linear"), 
    ...
    geom_line(data = . %>% filter(model_variant == "broken_linear"), 
              aes(x = year, y = prediction_recalc/10, linetype = `na 1993`), 
              alpha = 0.75, size = symboolgrootte - 0.5, color = pal[2]) +
  annotate(geom = "text", x = 1960, y = -15 , label = "1.8 mm/year", color = pal[2]) +
  annotate(geom = "text", x = 2003, y = 10, label = "2.9 mm/year", color = pal[2]) +
    annotate("text", label = "breakpoint", x = 1993, y = -20, color = pal[3], alpha = 0.7) +
    coord_cartesian(ylim = c(NA, NA)) +
    xlab("year") +
    ylab("sea level in cm related to NAP") +
    labs(subtitle = "Sea level (corrected for storm surge and nodal tide) \n broken linear model") +
    fitstyle +
    theme(
      strip.text.y = element_text(angle = 0)) +
    theme(legend.title = element_blank()) +
...
ggsave(filename = "figures/summary-plot.png", width = 6, height = 4, units = "in")
```