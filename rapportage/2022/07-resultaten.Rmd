...
:   De zeespiegelscenario's zijn relatief ten opzichte van de gemiddelde zeespiegelstand over de periode 1986 tot 2005. De zeespiegel was toen al 2 cm boven \acr{NAP}, het referentiepunt voor de getijstations. Om de scenario's te vergelijken met de metingen is een correctie uitgevoerd zodat de projecties uitgedrukt worden t.o.v. \acr{NAP} (\>2005). Hiertoe is de gemiddelde gemeten zeespiegel over 1986-2005 opgeteld bij de zeespiegelverandering (t.o.v. die periode) in de projectie.
...
predictions_all2 <- predictions_all %>% ungroup() %>%
    ...
    filter(correction_variant == "GTSM") %>%
    filter(model_variant %in% c("broken_linear")) %>% 
    select(year, model_variant, height, `height - anomaly`, prediction_recalc, `nodal tide`, pred_int_mean_tide_lower, pred_int_mean_tide_upper) %>% 
...
## Change:
## read from klimaatsignaal csv, only show that version. 
## remove all text and lines from the knmi22

knmi_klimaatsignaal <- read_csv("../../data/knmi/knmi21/DataKlimaatsignaal21_SeaLevel_2100_ref_period_1995_2014.csv") %>%
  select(time, ssp126_5pc, ssp126_50pc, ssp126_95pc)
...
# knmi_df <- knmi_df21 %>% bind_rows(knmi_df22)
  ...
  plotColors = c(
    # "zeespiegel" = "darkgrey",
    "KNMI klimaatsignaal 21 laag" = pal[1],
    # "zeespiegel (-opzet)" = pal[2],
  ...
  ggplot(predictions_all2) +
    ...
    #             alpha = 0.2) +
    geom_line(data = knmi_klimaatsignaal, aes(time, ssp126_50pc + (height_1995_2014), color = "KNMI klimaatsignaal 21 laag")) +
        geom_ribbon(
          data = . %>% filter(model_variant == "broken_linear"), 
                aes(x = year, 
                    ymin = pred_int_mean_tide_lower/10, 
                    ymax = pred_int_mean_tide_upper/10, 
                    fill = "predictie-interval"), 
          alpha = 0.2) +
    geom_ribbon(
      data = knmi_klimaatsignaal,
                aes(time, 
                    ymin = ssp126_5pc + (height_1995_2014), 
                    ymax = ssp126_95pc + (height_1995_2014),
                    fill = "5 - 95 percentiel"
                    ),
                alpha = 0.2) +
    coord_cartesian(xlim = c(1995, 2030), ylim = c(-5, 20)) +
    scale_color_manual(values = plotColors) +
    scale_fill_manual(values = plotFills) +
    xlab("jaar") +
    ylab("zeespiegel in cm tov NAP") +
