...
predictions_all %>% ungroup() %>%
    ...
    geom_point(aes(year, (`height - anomaly` - `nodal tide`)/10), alpha = 0.4, size = symboolgrootte) +
    ...
    geom_line(data = . %>% filter(model_variant == "broken_linear"), 
              ...
              alpha = 0.75, size = symboolgrootte, color = "blue") +
  annotate(geom = "text", x = 1960, y = -15 , label = "1.8 mm/jaar", color = "blue") +
  annotate(geom = "text", x = 2000, y = 10, label = "2.9 mm/jaar", color = "blue") +
    coord_cartesian(ylim = c(NA, NA)) +
    xlab("jaar") +
...
ggsave(filename = "figures/samenvatting_v2.png", width = 6, height = 4)
```
...
:   De zeespiegelscenario's zijn relatief ten opzichte van de gemiddelde zeespiegelstand over de periode 1986 tot 2005. De zeespiegel was toen al 2 cm boven \acr{NAP}, het referentiepunt voor de getijstations. Om de scenario's te vergelijken met de metingen is een correctie uitgevoerd zodat de projecties uitgedrukt worden t.o.v. \acr{NAP} (\>2005). Deze correctie is op twee manieren uitgevoerd.
    ...
    2\. De referentieperiode van de knmi '21 scenario's (1986-2005) is vervroegd naar de referentieperiode van het knmi '14 scenario (1980-2010). [methodiek hiervan staat in een rekendocument](documents/BackwardExtentionKS21.html)

Absoluut

:   De zeespiegelscenario's hebben betrekking op de absolute zeespiegel. Als we deze willen vergelijken met de getijdestations moeten we van de metingen de bodemdaling af halen
...
(ref:metingen-scenarios-label) Zeespiegelprojecties (mediaan laag scenario) met 5 en 95 percentiel uit \acr{KNMI} klimaatsignaal 2021 [@KNMI2021] gecorrigeerd voor referentieperiode en omgerekend naar NAP (methode 1 "NAP", methode 2 als "NAP + referentieperiode") vergeleken met metingen gemiddeld over de 5 hoofdstations en gecorrigeerd voor windopzet (\acr{GTSM}) en getij.

```{r metingen-scenarios, fig.cap="(ref:metingen-scenarios-label)"}

# read projection data

load("../../data/knmi/knmi21/knmi_df.Rdata")
knmi_df21 <- knmi_df %>%
  filter(scenario == "ssp126", percentiles %in% c(5, 50, 95), proc == "Total") %>%
  mutate(correction = "projectie (NAP)")

load("../../data/knmi/knmi22/knmi_df22.Rdata")
knmi_df22 <- knmi_df %>%
  filter(scenario == "ssp126", percentiles %in% c(5, 50, 95), proc == "Total") %>%
  mutate(correction = "projectie (NAP + referentieperiode)")

knmi_df <- knmi_df21 %>% bind_rows(knmi_df22) %>%
  mutate(perc_ts = perc_ts/10)
  ...
  plotColors = c(
    ...
    # "zeespiegel (-opzet)" = pal[2],
    "zeespiegel (-opzetanomalie -nodaal getij)" = pal[2],
    "projectie (NAP + referentieperiode)" = pal[3],
    "predictie (-opzetanomalie -nodaal getij)" = pal[4]
  )
  plotFills = c(
    "projectie (NAP)" = pal[1],
...
predictions_all2 <- predictions_all %>% ungroup() %>%
    ...
    filter(correction_variant == "GTSM") %>%
    filter(model_variant %in% c("broken_linear")) %>% 
    select(year, model_variant, height, `height - anomaly`, prediction_recalc, `nodal tide`) %>% 
...
height_1986_2005 <- predictions_all2 %>%
  ...
  ungroup() %>%
  summarize(mean = mean(`height`)/10) %>% unlist %>% unname

# height_1986_2005 <- predictions_all2 %>%
#   filter(year >= 1986 & year <= 2005) %>%
#   ungroup() %>%
#   summarize(mean = mean(`height-anomaly-tide`)) %>% unlist %>% unname

    ggplot(predictions_all2) +
    # geom_jitter(aes(year, `nodal tide`/10, color = model_variant), alpha = 0.75) +
    geom_point(aes(year, (`height`)/10, color = "zeespiegel (-opzetanomalie -nodaal getij)"), alpha = 0.4, size = symboolgrootte) +
    geom_line(data = . %>% filter(model_variant == "broken_linear"), 
              aes(x = year, y = prediction_recalc/10, color = "predictie (-opzetanomalie -nodaal getij)"), 
              alpha = 0.75, size = symboolgrootte) +
    geom_line(data = knmi_df %>% filter(percentiles == 50),
            aes(time, perc_ts + height_1986_2005, color = correction)
            ) +
  geom_ribbon(data = knmi_df %>% filter(percentiles != 50) %>%
                pivot_wider(id_cols = c(scenario, time, correction, proc), names_from = percentiles, values_from = perc_ts),
            aes(time, ymin = `5` + height_1986_2005, ymax = `95` + height_1986_2005, fill = correction),
            alpha = 0.2
            ) +
    coord_cartesian(xlim = c(1980, 2050), ylim = c(-10, 40)) +
     scale_color_manual(values = plotColors) +
    scale_fill_manual(values = plotFills) +
    xlab("jaar") +
    ylab("zeespiegel in mm tov NAP") +
...
In figuur \@ref(fig:metingen-scenarios) zien we dat de metingen afhankelijk van de correctie aan de onderkant ligt van de laagste projectie (correctie methode 1) of ongeveer gelijk aan de laagste projectie (correctie via gemodelleerde zeespiegel). In beide gevallen is de huidige stijging in de predictie hoger dan verwacht op grond van het gebroken lineaire model. Hier is dus een discrepantie. De resultaten in Figuur \@ref(fig:metingen-scenarios) laten zien dat de NAP-correctie methode een beperkte invloed heeft op de match tussen de scenario's en de langjarige trend in de metingen. De getoonde scenario's impliceren een relatief steile toename van de snelheid waarmee de zeespiegel in de nabije toekomst stijgt.
