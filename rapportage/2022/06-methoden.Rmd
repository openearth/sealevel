...
Er is een rijke diversiteit aan modellen beschikbaar om een korte termijn trendschatting te maken. Een uitgebreid overzicht van toepassingen in de context van zeespiegeltrends wordt gegeven door @Visser2015 [@Watson2016]. Er is weinig consistentie in de literatuur, zie bijvoorbeeld de recente toepassing van een [LSTM]{acronym-label="LSTM" acronym-form="singular+short"} [@Braakmann-Folgmann2017], een state space model [@Frederikse2016] en een [STL]{acronym-label="STL" acronym-form="singular+short"} [@Breaker2011] aanpak. Er is een aantal instituten die een actueel zeespiegelcijfer rapporteren, met name [AVISO]{acronym-label="AVISO" acronym-form="singular+short"}, [CSIRO]{acronym-label="CSIRO" acronym-form="singular+short"}, [NASA]{acronym-label="NASA" acronym-form="singular+short"} en de [NOAA]{acronym-label="NOAA" acronym-form="singular+short"}. Zij rapporteren een zeespiegeltrend op basis van een lineaire trend. Ook in het Compendium voor de Leefomgeving [@CBS2018] wordt de zeespiegel als lineare trend gerapporteerd.

In dit onderzoek maken we gebruik van een [GLM]{acronym-label="GLM" acronym-form="singular+short"}, een gegeneralizeerde vorm van een lineair regressiemodel. Vooral omdat dit opvallend goed werkt [@Watson2016] en omdat er geen modellen zijn die zich, op basis van bovenstaande criteria, als betere hebben bewezen. Omdat we er vanuit gaan dat de zeespiegel sneller aan het stijgen is of binnenkort sneller gaat stijgen maken we ook gebruik van schattingen van de versnelling, zie [4.2](#sec:versnelling-methods){reference-type="ref" reference="sec:versnelling-methods"}. Er wordt ook gebruik gemaakt van een Bayesiaanse [MCMC]{acronym-label="MCMC" acronym-form="singular+short"} variant. Dit sluit ook aan bij de aanbevelingen om naar een multimodel aanpak [@Baart2012f; @Visser2015] over te gaan. Merk op dat een lineair model niet impliceert dat de zeespiegelstijging een rechte lijn moet volgen. Ook polynome, sinusoïde, exponentiële en loglineaire modellen vallen onder de [GLM]{acronym-label="GLM" acronym-form="singular+short"} familie.

## Welke data? {#sec:bronnen}

In dit onderzoek maken we gebruik van de 6 zogenaamde hoofdstations Vlissingen, Hoek van Holland, IJmuiden, Den Helder, Harlingen en Delfzijl. Deze kuststations leveren sinds 1890, toen het [NAP]{acronym-label="NAP" acronym-form="singular+short"} overal was doorgevoerd, betrouwbare metingen. In De Ronde et al. (2013) is de methode Zeespiegelmonitor op de 6 hoofdstations toegepast. Er is overwogen om de stations uit te breiden met andere stations uit het overzicht in figuur \@ref(fig:waterbase) 
...
De methode in dit onderzoek bouwt verder op de aanpak zoals beschreven in @Baart2015. De variatie jaargemiddelde zeespiegel wordt beschreven met behulp van een [GLM]{acronym-label="GLM" acronym-form="singular+short"}. In deze sectie beschreven we welke termen in dit model meegenomen worden en waarom. 
...
De versnelling is optioneel (zie sectie \@ref(versnelling-methods). In \@ref(eq:gedetailleerdevergelijking)  is de gedetailleerde wiskundige vergelijking van het regressiemodel voor de zeewaterstand
opgenomen.

## Constante en trend

Meestal kijken we naar zeespiegeltrends en naar veranderingen in de trends. Maar eigenlijk net zo interessant is hoe hoog de zeespiegel precies staat. Het nulniveau van het [NAP]{acronym-label="NAP" acronym-form="singular+short"} begon ooit als hoogwater in Amsterdam, toen stond het gemiddeld zeeniveau dus onder [NAP]{acronym-label="NAP" acronym-form="singular+short"} [@Veen1945]. Begin jaren 1980 was de zeespiegel gelijk aan het [NAP]{acronym-label="NAP" acronym-form="singular+short"} en tegenwoordig ligt [NAP]{acronym-label="NAP" acronym-form="singular+short"} onder het gemiddeld zeeniveau.
...
De uiteindelijke modelkeuze (welk model is beter?) vindt plaats op twee criteria. Er moet sprake zijn van een significante acceleratie of deceleratie en er moet sprake zijn van een beter model. Dat laatste wordt vastgesteld met behulp van het [AIC]{acronym-label="AIC" acronym-form="singular+short"}, waarbij een afweging wordt gemaakt tussen de complexiteit en de toegevoegde waarde.

Naast de bepaling of er versnelling is die aansluit bij de literatuur gebruiken we ook nog een andere methode om een eventueel knikpunt te bepalen. In dit model kan een breekpunt liggen tussen 1900 en 2000. Er is een trend voor en een trend na het breekpunt. Deze aanpak staat bekend als een [MCMC]{acronym-label="MCMC" acronym-form="singular+short"} analyse [vergelijk de 'coal mine' case study uit @Salvatier2016]. Deze methode stelt vast of er een significante knik is, wanneer die knik het meest waarschijnlijk plaats vond ([MAP]{acronym-label="MAP" acronym-form="singular+short"}) en of we een knik hadden gevonden als die er wel geweest was, zie ook de gedetailleerde uitwerking .
...
Omdat de Nederlandse kust aan een continentaal plat ligt, waar de wind voor een grote opslingering kan zorgen, maken we gebruik van windgegevens. De heranalyses (een combinatie van modellen en historische gegevens) van de [NCEP]{acronym-label="NCEP" acronym-form="singular+short"} [@Kalnay1996], op basis van een punt op de Noordzee (lat: 52.38, lon: 3.75), worden gebruikt als basis voor de schatting van de windopzet. De maandgemiddelde Noord-Zuid en Oost-West component worden gebruikt. Ten opzichte van @Baart2015, waar deze methode in detail beschreven staat, worden de gegevens ongeroteerd gebruikt om de gegevens generieker toepasbaar te maken. Wind heranalyses zijn beschikbaar vanaf 1949.

Naast bovenstaande modelspecificatie is ook gekeken of de windopzetten uit het heranalyses van [GTSM]{acronym-label="GTSM" acronym-form="singular+short"} gebruikt kunnen worden. Het doorrekenen van wind naar de kust met een hydrodynamisch model heeft als voordeel dat getij-stromingsinteractie, het inverse barometer effect en stormopzet onderscheiden kunnen worden. Als alternatief zou ook het [DCSM]{acronym-label="DCSM" acronym-form="singular+short"} gebruikt kunnen worden, zie sectie \@ref(frederikse). 

## Residu

Een ander aspect is de autocovariantie tussen de achtereenvolgende jaargemiddelde zeespiegelstanden. Als wind niet in de vergelijking wordt meegenomen is de autocovariantie negeerbaar. Dit jaar voegen we aan de vergelijking toe dat we autocorrelatie meenemen als daar aanleiding voor is (Durbin Watson score ruim onder de 2). In de literatuur is dit beschreven door bijvoorbeeld @Bos2014. Zij adviseren om voor jaargemiddelden een [AR]{acronym-label="AR" acronym-form="singular+short"} term mee te nemen, net name een [AR]{acronym-label="AR" acronym-form="singular+short"}1 term, rekening houdend met 1 jaar terug. Zie ook @Visser2015 voor een nadere discussie.
...
Voor de term $versnelling?$ kan in
vergelijking \@ref(eq:gedetailleerdevergelijking) de volgende term ingevuld worden in het
geval van gebroken lineaire trend: 
