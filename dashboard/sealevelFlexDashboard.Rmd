...
### Exclude processes from observations and projections
...
renderPlotly({
  ...
  if (input$`fitType` == 'loess') {
    ...
    fig <- fig  + geom_smooth(method = "loess", span = span, alpha = showSE, color = "coral2")
  }
  ...
  fig <- fig + 
    geom_line(data = knmi_mapped2, mapping = aes(year, height))
  ...
  if(length(input$exclude) != 0){
    fig = fig +
      geom_point(data = sealevelPlus2, aes(year, value), color = "darkolivegreen2", size = 2, alpha = 0.6) +
      geom_line(data = sealevelPlus2, aes(year, value), color = "darkolivegreen2", size = 2, alpha = 0.6) +
      geom_line(data = knmi_mapped2, aes(year, value), color = "darkolivegreen2", size = 2, alpha = 0.6)
  }
