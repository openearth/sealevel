...
renderPlotly({
  ...
  if (input$`fitType` == 'loess') {
    ...
    fig <- fig  + geom_smooth(span = span, alpha = showSE)
  }
  ...
  if(length(input$exclude) != 0){
    fig = fig +
      geom_point(data = sealevelPlus2, aes(year, value), size = 2, alpha = 0.3) +
      geom_line(data = sealevelPlus2, aes(year, value), size = 2, alpha = 0.3) +
      geom_line(data = knmi_mapped2, aes(year, value), size = 2, alpha = 0.3)
  }
