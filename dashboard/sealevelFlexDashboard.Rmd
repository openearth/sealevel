...
observation_mapping <- list(
  ...
  `Terrestrial Water Storage [mean]` = "land",	
  `GIA [mean]` = "gia", 
...
recode(unique(knmi_perc50_df$proc), !!!projection_mapping)
...
fig <- sealevelPlus %>% 
  select(
    year, steric, glaciers, greenland, antarctica, land # gia and surge removed. not in scenarios
  ...
  pivot_longer(cols = c(steric, glaciers, greenland, antarctica, land),
