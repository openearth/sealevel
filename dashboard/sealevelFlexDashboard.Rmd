...
knmi_df <- read_csv(url, comment = "#") %>%
  mutate(perc_ts = 10*perc_ts)                        # everything in mm, rounded to 0 decimals
...
# merge 10, 50, and 90 percentiles
knmi_perc_df <- knmi_df %>% 
  filter(percentiles %in% c(10, 50, 90) & proc == "Total") %>%
  pivot_wider(
...
plotColorColors = c(
  "Observed sea level w standard error" = "#572024",
...
scenarioPopoverContent = paste("rcp45 = IPCC scenario SSP2-4.5",
                          "rcp85 = IPCC scenario SSP5-8.5",
                          sep = "<br>")
...
output$combinedPlot <- renderPlotly({
  ...
  sealevelPlus2 <- sealevelPlus %>% 
    mutate(subtract = rowSums(across(input$exclude))) %>% 
  ...
  knmi_mapped2 <- knmi_mapped %>%
    ...
    left_join(
      knmi_perc_df %>% select(year = time, scenario, 
                              ...
                              `10th percentile` = perc_ts_10, 
                              `90th percentile` = perc_ts_90
      )
    ) %>%
    mutate(subtract = rowSums(across(input$exclude))) %>% 
    mutate(value = `median height` - subtract) %>%
    mutate(across(c(value, `median height`, `10th percentile`, `90th percentile`), signif, digits = 3))
  ...
  fig <- fig + 
    ...
    annotate("text", label = "NAP", x = 2100, y = 25, size = 6, color = "bisque4")
  ...
  if(input$`fitType` == "linear"){
    ...
    # fig <- fig + annotate("text", label = paste("linear trend =", format(slope, digits = 4), "mm/year; ", "p=", pvalue), x = 1960, y = 100, size = 4)
  ...
  fig <- fig + geom_point(mapping = aes(color = "Observed sea level w standard error"))
  ...
  fig = fig + 
    ...
    ylab("sea level in mm relative to NAP") +
    theme(
