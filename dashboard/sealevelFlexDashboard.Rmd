...
sealevelAllStationsurl <-
"https://raw.githubusercontent.com/openearth/sealevel/feature/tide-update/data/deltares/results/dutch-sea-level-monitor-export-2022-01-14.csv"

sealevelAllStations <- read_csv(sealevelAllStationsurl,
                     comment = "#")

sealevelAllStations$`sea_surface_height_due_to_tide` = sealevelAllStations$`sea_surface_height_due_to_tide` - mean(sealevelAllStations$`sea_surface_height_due_to_tide`)
...
# observed_components$`sea_surface_height_due_to_tide [mm]` = observed_components$`sea_surface_height_due_to_tide [mm]` - mean(observed_components$`sea_surface_height_due_to_tide [mm]`)

observed_components$`Surge [mm]` = observed_components$`Surge [mm]` - mean(observed_components$`Surge [mm]`)

observed_components <- observed_components %>% 
  left_join(sealevelAllStations %>% select(year, sea_surface_height_due_to_tide), by = c(Year = "year"))
...
# rm(scenarios)
# 
# save(knmi_df, file =  file.path(datadir, "knmi_df.Rdata"))

load(file.path(datadir, "knmi_df.Rdata"))
...
projection_mapping <- list(
  `Global steric` = "other",  
  `Local ocean` = "surge", #"steric",  # this is local steric and wind combined
...
fillPage(
  fillCol(height = "80%", flex = c(NA,1), 
          h3("Dutch Sea Level Visualization."),
          actionButton(
            "clickMe",
            label = "Proceed", class = "btn btn-lg btn-primary",
            onclick="location.href='#section-dutch-sea-level';"
          )
  )
)
...
This app visualizes the sea level at the Dutch coast. Underlying data will be updated with new versions when available.
...
>The scenario dataset represent sea level anomalies with respect to the period 1986-2005. To align these data with the observations, the mean observed anual sea level over  the period 1986-2005 was added (>2005) to the KNMI scenarios. This the conversion from "anomalies" to water level related to NAP ("Nieuw Amsterdams Peil", Dutch sea level reference).
