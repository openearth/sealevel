...
"sea level excl selected processes" = "lightblue",
...
renderPlotly({
  ...
  # Show measurements
  fig <- fig + geom_point(
    mapping = aes(color = "average sea level")
    )
  ...
  if(length(input$exclude) != 0){
    fig = fig +
      geom_point(data = sealevelPlus2, aes(year, value, color = "sea level excl selected processes"), size = 3, alpha = 0.8) +
      # geom_line(data = sealevelPlus2, aes(year, value, color = "observation processes"), size = 2, alpha = 0.8) +
      geom_line(data = knmi_mapped2, aes(year, value, color = "sea level excl selected processes"), size = 2, alpha = 0.8)
  }
